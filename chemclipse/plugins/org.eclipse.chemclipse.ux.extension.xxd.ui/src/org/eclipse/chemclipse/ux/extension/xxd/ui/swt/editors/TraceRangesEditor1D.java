...
import org.eclipse.chemclipse.ux.extension.xxd.ui.internal.provider.TraceRange1DInputValidator;
...
public class TraceRangesEditor1D extends Composite implements IChangeListener, IExtendedPartUI {
	...
	public static final String EXAMPLE_ENTRY = "1.0 | 3.0 | Name | 104.02+-0.03";

	private AtomicReference<TraceRanges1DListUI> listControl = new AtomicReference<>();
	...
	public TraceRangesEditor1D(Composite parent, int style) {

		super(parent, style);
		createControl();
	}

	public void setInput(TraceRanges1D traceRanges) {

		this.traceRanges = (traceRanges == null) ? new TraceRanges1D() : traceRanges;
		setInput();
	}
	...
	public TraceRanges1D getTraceRanges() {
	...
	private void createTableSection(Composite parent) {
		...
		TraceRanges1DListUI stackRangesListUI = new TraceRanges1DListUI(parent, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
	...
	private void createButtonAdd(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				InputDialog dialog = new InputDialog(button.getShell(), DIALOG_TITLE, MESSAGE_ADD, EXAMPLE_ENTRY, new TraceRange1DInputValidator(traceRanges));
				if(IDialogConstants.OK_ID == dialog.open()) {
					String item = dialog.getValue();
					TraceRange1D setting = traceRanges.extractTraceRange(item);
					if(setting != null) {
	...
	private void createButtonEdit(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				TraceRange1D traceRange = getTraceRangeSelected();
				if(traceRange != null) {
					InputDialog dialog = new InputDialog(button.getShell(), DIALOG_TITLE, MESSAGE_ADD, traceRanges.extractTraceRange(traceRange), new TraceRange1DInputValidator(traceRanges));
					if(IDialogConstants.OK_ID == dialog.open()) {
						String item = dialog.getValue();
						TraceRange1D setting = traceRanges.extractTraceRange(item);
						if(setting != null) {
	...
	private void createButtonRemove(Composite parent) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				List<TraceRange1D> traceRangesRemove = new ArrayList<>();
				for(Object object : listControl.get().getStructuredSelection().toArray()) {
					if(object instanceof TraceRange1D traceRange) {
	...
	private TraceRange1D getTraceRangeSelected() {

		if(listControl.get().getStructuredSelection().getFirstElement() instanceof TraceRange1D traceRange) {
			return traceRange;
		}
