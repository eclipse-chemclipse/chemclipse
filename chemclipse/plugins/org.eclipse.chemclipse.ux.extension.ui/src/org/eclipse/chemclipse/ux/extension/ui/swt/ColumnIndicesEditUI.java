...
import org.eclipse.chemclipse.ux.extension.ui.dialogs.ColumnIndexMarkerEditDialog;
...
public class ColumnIndicesEditUI extends Composite {
	...
	public ColumnIndicesEditUI(Composite parent, int style) {
	...
	private void addColumnIndexMarker() {

		if(libraryInformation != null) {
			ColumnIndexMarkerEditDialog dialog = new ColumnIndexMarkerEditDialog(getShell(), null);
			if(dialog.open() == Window.OK) {
				IColumnIndexMarker marker = dialog.getColumnIndexMarker();
				if(marker != null) {
	...
	private void editColumnIndexMarker() {

		if(libraryInformation != null) {
			IStructuredSelection selection = listControl.get().getStructuredSelection();
			if(selection.size() == 1 && selection.getFirstElement() instanceof IColumnIndexMarker existing) {
				ColumnIndexMarkerEditDialog dialog = new ColumnIndexMarkerEditDialog(getShell(), existing);
				if(dialog.open() == Window.OK) {
					IColumnIndexMarker updated = dialog.getColumnIndexMarker();
					if(updated != null) {
	...
	private void deleteColumnIndexMarkers() {

		if(libraryInformation != null) {
			IStructuredSelection selection = listControl.get().getStructuredSelection();
			for(Object object : selection.toList()) {
				if(object instanceof IColumnIndexMarker marker) {
