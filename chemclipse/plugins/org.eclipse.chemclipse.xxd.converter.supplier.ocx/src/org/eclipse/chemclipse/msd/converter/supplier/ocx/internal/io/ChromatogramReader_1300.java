...
public class ChromatogramReader_1300 extends AbstractChromatogramReader {
	...
	private IChromatogramMSD readZipData(Object object, String directoryPrefix, File file, IProgressMonitor monitor) throws IOException {
		...
		try {
			...
			readPeaks(getDataInputStream(object, directoryPrefix + Format.FILE_PEAKS_MSD), closeStream, chromatogram);
			readArea(getDataInputStream(object, directoryPrefix + Format.FILE_AREA_MSD), closeStream, chromatogram);
			subMonitor.worked(20);
			readIdentification(getDataInputStream(object, directoryPrefix + Format.FILE_IDENTIFICATION_MSD), closeStream, chromatogram);
			readHistory(getDataInputStream(object, directoryPrefix + Format.FILE_HISTORY_MSD), closeStream, chromatogram);
			subMonitor.worked(20);
			readMiscellaneous(getDataInputStream(object, directoryPrefix + Format.FILE_MISC_MSD), closeStream, chromatogram);
			readSeparationColumn(getDataInputStream(object, directoryPrefix + Format.FILE_SEPARATION_COLUMN_MSD), closeStream, chromatogram);
	...
	private void readScanProxies(ZipFile zipFile, String directoryPrefix, File file, IChromatogramMSD chromatogram, IProgressMonitor monitor) throws IOException {
		...
		DataInputStream dataInputStream = getDataInputStream(zipFile, directoryPrefix + Format.FILE_SCANPROXIES_MSD);
		int scans = dataInputStream.readInt();
		...
		for(int scan = 1; scan <= scans; scan++) {
			...
			IVendorScanProxy massSpectrum = new VendorScanProxy(file, offset, Format.CHROMATOGRAM_VERSION_1300, ionTransitionSettings, monitor);
			massSpectrum.setRetentionTime(retentionTime);
	...
	private void readScans(DataInputStream dataInputStream, boolean closeStream, IChromatogramMSD chromatogram, IProgressMonitor monitor) throws IOException {
		 ...
		 * Scans
		 */
		IReaderProxy readerProxy = new ReaderProxy_1300();
		int scans = dataInputStream.readInt();
	...
	private void parseChromatogram(Object object, String dataType, String directoryPrefix, IChromatogramMSD chromatogram, IProgressMonitor monitor) throws IOException {
	...
	private boolean isValidFileFormat(ZipFile zipFile) throws IOException {
		...
		if(version.equals(Format.CHROMATOGRAM_VERSION_1300)) {
			isValid = true;
		}
